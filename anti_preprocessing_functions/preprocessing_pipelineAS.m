...
% Updated 2.14.2025:
%   renaming trials if dropped/correct/incorrect/error corrected
...
renamedtrial_folder = 'renamedtrials';
...
if isstruct(xEEG)
    ...
    %% Rename EEG.events.types to correct, incorrect, error corrected, or dropped
    % redoing lines 118-170 to include error corrected trials which were
    % not identified in first preprocessing pipeline
    % Going to keep all trials for preprocessing pipeline, can later epoch to specific types of trials
    addpath('/Volumes/Hera/Projects/7TBrainMech/scripts/eeg/eog_cal') % need to load AS score data table
    load('eeg_anti_20250218.mat','datatable')
    subsetdatatable = datatable(:,{'LunaID','ScanDate','Trial','Correct','trialOnsetIndex'});
    % just looking at subject's data table
    subIdx = subsetdatatable{:,'LunaID'} == subid & subsetdatatable{:,'ScanDate'} == scandate;
    subTrialTable = subsetdatatable(subIdx,{'LunaID','ScanDate','Trial','Correct','trialOnsetIndex'});
    % error if no trial data table
    if isempty(subTrialTable)
        error('\nMissing Subject Trial Table\n')
    end
    ...
    EEG = pop_saveset(EEG,'filename',[currentName,'_renamedtrials'],'filepath',fullfile(outpath,renamedtrial_folder));
    [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG,EEG);
